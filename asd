...
void Client :: checkLogin() {
    if (this->_nick == "\1" || this->_user == "" || this->_pass != this->_serverPass)
        return;
...
void Client :: Priv(IrcMessage &msg) {
    ...
    if (params[0][0] == '#') {
        ...
        if (this->_channels.find(ch) == this->_channels.end()) {
            string ret = ":ka100 403 " + ch+ " :Server not in Server\r\n";
            this->writeClient(ret); 
            return;
        }
...

{
    ...
    while (true)
    {
        if (!this->run)
            break;
        int pollResult = poll(this->_conn.connectionSockets.data(), this->_conn.getSize(), -1);
        ...
        for (list<Client *>::iterator it = this->_conn.connectionList.begin(); it != this->_conn.connectionList.end() && this->run;)
        {
            int readResult = (*it)->readClient(buff);
            if (readResult == 1)
            {
                size_t index = 0;
                for (list<Client *>::iterator listIt = this->_conn.connectionList.begin(); listIt != this->_conn.connectionList.end(); ++listIt, ++index)
                    if (*listIt == *it)
                        break;

                // Removendo o cliente da lista
                it = this->_conn.connectionList.erase(it);

                // Removendo o socket correspondente do vetor connectionSockets
                vector<pollfd>::iterator socketIt = this->_conn.connectionSockets.begin() + index;
                this->_conn.connectionSockets.erase(socketIt);
        ...
        cout << "Size of list: " << this->_conn.connectionList.size() << endl;
    }
    return false;
}